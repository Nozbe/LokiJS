...
(function (root, factory) {
  ...
  if (typeof define === "function" && define.amd) {
    ...
    // Browser globals
    (root as any).IncrementalIndexedDBAdapter = factory();
  }
})(this, function () {
  return (function () {
    "use strict";
    ...
    /* jshint -W030 */
    var DEBUG = typeof window !== 'undefined' && !!(window as any).__loki_incremental_idb_debug;
    ...
    function IncrementalIndexedDBAdapter(options) {
      ...
      if (shouldSetupEncryption) {
        ...
        this.decrypt = makeExternalFunctionSafe(
          ...
          function (a) { return 'Error while invoking decrypt function. Is the data really encrypted? Supplied args: ' + a; }
        );
    ...
    IncrementalIndexedDBAdapter.prototype._getChunk = function (collection, chunkId) {
      ...
      var firstDataPosition: number | null = null;
      ...
      var lastDataPosition: number | null = null;
     ...
     */
    IncrementalIndexedDBAdapter.prototype.saveDatabase = async function (dbname, getLokiCopy, callback) {
      var that = this;
      ...
      try {
        ...
        const initialLokiVersionId = await that.getLokiVersionId()
        // note that initialPureLokiObject can be encrypted
        const initialPureLokiObject: any = await idbRequestResult(that.getPureLokiObjectRequest())
        ...
        if (initialLokiVersionId !== that._prevLokiVersionId) {
          ...
          const allKeys = await idbRequestResult(store.getAllKeys())
          
          maxChunkIds = await getMaxChunkIds(allKeys, that.keyResolver);
        }

        const incremental = !maxChunkIds;

        const chunkInfo = await that._getLokiChanges(getLokiCopy(), incremental, maxChunkIds);
        ...
        try {
          idbReq(that.getPureLokiObjectRequest(tx), function(e) {
            ...
            const lokiObjectsMatch = (latestPureLokiObject === undefined && initialPureLokiObject === undefined)
              || (that.encryptOutputsString && latestPureLokiObject?.value === initialPureLokiObject?.value)
              || (!that.encryptOutputsString && arrayBuffersEqual(latestPureLokiObject?.value, initialPureLokiObject?.value))
            if (lokiObjectsMatch) {
    ...
    async function getMaxChunkIds(allCiphertextKeys, keyResolver) {
      const maxChunkIds = {};
      for(let ciphertextKey of allCiphertextKeys) {
        // table.chunk.2317
        if (await keyResolver.isChunkKey(ciphertextKey)) {
    ...
    IncrementalIndexedDBAdapter.prototype.getLokiVersionId = async function(tx=undefined) {
      if (!tx) {
        tx = this.idb.transaction([this.idbActualLokiObjectStoreName], "readwrite");
      }
      ...
      const chunk = await idbRequestResult(store.get(this.keyResolver.actualLokiKey))

      return await lokiChunkVersionId(chunk, this.decrypt);
    }

    async function lokiChunkVersionId(chunk, decryptFn) {
      try {
        if (chunk) {
          var loki = JSON.parse(await decryptFn(chunk.value));
    ...
    IncrementalIndexedDBAdapter.prototype._getLokiChanges = async function (loki, incremental, maxChunkIds) {
      var that = this;
      var collectionVersionIds: { name: any, versionId: any; }[] = [];
      var savedSize = 0;

      var prepareCollection = async function (collection, i) {
        ...
        var prepareChunk = async function (chunkId) {
          ...
          chunkData = await that.encrypt(JSON.stringify(chunkData));
          ...
          try {
            putChunks.push({
              key: await that.keyResolver.getOrGenerateCiphertextCollectionChunkKey(collection.name, chunkId),
              value: chunkData,
            })
          } catch(e) {
            console.error(e)
        ...
        if (incremental) {
          ...
          for (var k = maxChunkId + 1; k <= persistedMaxChunkId; k += 1) {
            var deletedChunkName = await that.keyResolver.getOrGenerateCiphertextCollectionChunkKey(collection.name, k);
        ...
        if (collection.dirty || dirtyChunks.size || !incremental) {
          ...
          var metadataChunk = await that.encrypt(JSON.stringify(collection));
          ...
          putChunks.push({
            key: await that.keyResolver.getOrGenerateCiphertextCollectionMetadataKey(collection.name),
            value: metadataChunk,
          })
        }
      ...
      const collectionsChanges = await Promise.all(loki.collections.map((collection, index) => prepareCollection(collection, index)))
      loki.idbVersionId = randomVersionId();
      var serializedMetadata = await that.encrypt(JSON.stringify(loki));
    ...
    IncrementalIndexedDBAdapter.prototype._applyDBChanges = async function (idbStore, dbChanges) {
     ...
     * @memberof IncrementalIndexedDBAdapter
     */
    IncrementalIndexedDBAdapter.prototype.loadDatabase = async function (dbname, callback) {
      var that = this;
      ...
      await this._getAllChunks(dbname, async function (chunks) {
        try {
          ...
          chunks = await chunksToMap(chunks, that.keyResolver);
    ...
    async function chunksToMap(chunks, keyResolver) {
      var loki;
      var chunkMap = {};
      await sortChunksInPlace(chunks, keyResolver);
      for(let object of chunks){
        var key = (object as any).key;
        var value = (object as any).value;
        if (keyResolver.isLokiKey(key)) {
          loki = value;
          continue;
        } else {
          if (await keyResolver.isChunkKey(key)) {
    ...
    IncrementalIndexedDBAdapter.prototype._initializeIDB = async function (dbname, onError, onSuccess) {
      var that = this;
      ...
      const encryptedObjectStoreName = await that._encryptToString(that._names.objectStoreName);

      var openRequest = indexedDB.open(dbname, 1);

      openRequest.onupgradeneeded = function (e) {
        var db = (e.target as any).result;
      ...
      openRequest.onsuccess = function (e) {
        ...
        (async() => {
          const idbActualLokiObjectStoreName = await findIdbActualLokiObjectStoreName(objectStoreNames, that._names.objectStoreName, that._decryptFromString.bind(that))
          ...
          const keyResolver = await extractkeyResolver(store, that._names, that.encrypt, that._encryptToString.bind(that),
            that.decrypt, that._decryptFromString.bind(that))
    ...
    IncrementalIndexedDBAdapter.prototype._getAllChunks = async function (dbname, callback) {
      var that = this;
      if (!this.idb) {
        this._initializeIDB(dbname, callback, function () {
      ...
      async function getMegachunks(keys) {
        ...
        async function processMegachunk(megachunk, megachunkIndex, keyRange) {
          ...
          for(let i=0; i<megachunk.length; i++) {
            try {
              ...
              await parseChunk(chunk, that.deserializeChunk, that.keyResolver, that.decrypt, that._decryptFromString.bind(that));
        ...
        async function requestMegachunk(index) {
      ...
      function getAllChunks() {
        idbReq(store.getAll(), function (e) {
          ...
          (async() => {
            await Promise.all(allChunks.map(async function (chunk) {
              await parseChunk(chunk, that.deserializeChunk, that.keyResolver, that.decrypt, that._decryptFromString.bind(that));
            }));
            callback(allChunks);
          })()
    ...
    async function parseChunk(chunk, deserializeChunk, keyResolver, decryptFn: (value) => Promise<any>, decryptFromStringFn: (value) => Promise<any>) {
      if (typeof chunk.value === 'string') {
        chunk.value = JSON.parse(await decryptFromStringFn(chunk.value));
      } else {
        chunk.value = JSON.parse(await decryptFn(chunk.value));
      }

      if (deserializeChunk) {
        if (await keyResolver.isChunkKey(chunk.key)) {
    ...
    IncrementalIndexedDBAdapter.prototype._encryptToString = async function (s) {
      const encrypted = await this.encrypt(s)
    ...
    IncrementalIndexedDBAdapter.prototype._decryptFromString = async function (s) {
      if (this.encryptOutputsString) {
    ...
    async function _getSortKey(object, keyResolver) {
      var key = object.key;
      if (await keyResolver.isChunkKey(key)) {
    ...
    async function sortChunksInPlace(chunks, keyResolver) {
      ...
      const mapped = new Map(await Promise.all(chunks.map(async item => {
        const sortKey = await _getSortKey(item, keyResolver);
        return [item.key, sortKey];
      })));
      
      chunks.sort(function (a, b) {
        const aKey = mapped.get(a.key),
          bKey = mapped.get(b.key);
        if (!bKey) return 1;
        if (!aKey) return -1;
    ...
    async function findIdbActualLokiObjectStoreName(objectStoreNames, decryptedObjectStoreName, decryptFromStringFn) {
      var domStringList = objectStoreNames;
      for (var i = 0; i < domStringList.length; i++) {
        if (await decryptFromStringFn(domStringList[i]) === decryptedObjectStoreName) {
    ...
    async function doNothing(x): Promise<any> { return x; }
    ...
    async function isCiphertextChunkKey(ciphertextChunkKey, _names, decryptFn, decryptFromStringFn) {
      ...
      var plaintextChunkKeyWithoutChunkId = await decryptFromStringFn(ciphertextChunkKeyWithoutChunkId);
      ...
      if (isEncriptionNotEnabled) {
        return new Promise(function (resolve) {
          resolve({
            ...
            isChunkKey: async function (plaintextKey) {
            ...
            getOrGenerateCiphertextCollectionMetadataKey: async function (collectionName) {
            ...
            getOrGenerateCiphertextCollectionChunkKey: async function (collectionName, chunkId) {
        ...
        idbReq(store.getAllKeys(), function (e) {
          ...
          (async() => {
            ...
            if (keys.length === 0) {
              ...
              ciphertextLokiKey = await encryptToStringFn(_names.lokiKeyName);
            } else {
              for (var i = 0; i < keys.length; i++) {
                const key = keys[i];
                if (await isCiphertextChunkKey(key, _names, decryptFn, decryptFromStringFn)) {
                  ...
                  const plaintextCollectionName = extractCollectionNameFromPlaintextChunkKeyWithoutChunkId(await decryptFromStringFn(ciphertextChunkKeyWithoutChunkId), _names);
                  ...
                  var plaintextKey = await decryptFromStringFn(key);
            ...
            if (ciphertextLokiKey) {
              resolve({
                ...
                isChunkKey: async function (ciphertextKey) {
                  return await isCiphertextChunkKey(ciphertextKey, _names, decryptFn, decryptFromStringFn);
                },
                ...
                getOrGenerateCiphertextCollectionMetadataKey: async function (collectionName) {
                  if (!keyResolver.ciphertextCollectionMetadataKeys[collectionName]) {
                    const metadataCiphertext = await encryptToStringFn(collectionName + "." + _names.metadata);
                ...
                getOrGenerateCiphertextCollectionChunkKey: async function (collectionName, chunkId) {
                  if (!plaintextCollectionNamesToCiphertextChunkKeysWithoutChunkId[collectionName]) {
                    const ciphertextCollectionNameAndChunk = await encryptToStringFn(collectionName + "." + _names.chunk);
